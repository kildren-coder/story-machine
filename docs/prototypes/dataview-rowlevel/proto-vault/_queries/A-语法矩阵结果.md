...
## 记录处 · 判读结论（2026-07-27 跑通）

| 变体 | 结果 |
|---|---|
| V1 四空格续行（SPEC §5.1 原方案） | ❌ 字段没挂上父 list item |
| V2 Tab 续行 | ❌ 同上 |
| V3 两空格续行 | ❌ 没挂上，且泄漏成了页面级属性（见 A-3） |
| V4 零缩进续行 | ❌ 没挂上，且泄漏成了页面级属性（见 A-3） |
| V5 嵌套 list | ❌ 挂不上父行，还炸出 10 条碎行（16 条总条目里 11 条来自 V5，见 A-2） |
| V6 全挤一行括号式 | ✅ 唯一挂满全部字段的写法（A-4 验证通过） |

**A-3 补充**：页面级 `type`/`情态` 各聚合出 2 条值（而非预期的 1 条），说明续行失败不止是"字段消失"，
V3、V4 两种写法的字段都泄漏到了整篇笔记的属性层——这是比丢字段更隐蔽的失败模式：单看渲染以为成功，
一做跨集查询才发现一集只剩一条"断言"。

**结论：SPEC §5.1 现方案（缩进续行）不成立，改用 V6（`[key:: value]` 全部挤一行）。**
已同步：`SPEC.md` §5.1 改写为括号式，§11 第一条开放问题划掉。
