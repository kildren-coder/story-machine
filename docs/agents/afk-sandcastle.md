...
- **一次运行的量**：约一个 PR、几百行 diff 以内；设计取舍已在父票 / wayfinder
  grilling 阶段定死，不留开放问题。描述里有「顺便 / 以及 / 同时」，先拆。
...
- **Fixture check（出题时逐票必问）**：这张票的正确性是否取决于输入数据的形状
  ...
  像样的样例，这一步只能发生在出题侧。
- **四问（出题时逐票必问）**：四个维度逐项给出「已回答（答案写进票面或 SPEC）」
  或「显式不适用」，有一项答不出就不打 `ready-for-agent`：
