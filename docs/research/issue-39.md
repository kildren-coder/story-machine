...
| **键归一化** | `canonicalizeVarName`：转小写、空格转连字符、去无效字符；原名与归一名都存 | 查询时大小写不敏感；程序读要认归一名 |
