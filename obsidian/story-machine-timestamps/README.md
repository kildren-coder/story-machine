...
`timestamp-player` 的按钮绑定音频靠 **DOM 位置**——`findAudioForBtn` 沿文档顺序找「我上面最近的那个 `<audio>`」，和这条断言出自哪一集无关。在跨集查询表里，整张表会全播同一个音频：点 EP07 的 `04:00`，播的是 EP01 的 4 分 00 秒，**有声音、位置也对，就是集数错了，还不报错**。另有两个坑：正则只认 `MM:SS`，`00:29:00` 被解析成 29 **秒**（SPEC §5.3 用的正是 `HH:MM:SS`，静默错 60 倍）；`activeBtn` 跟播放进度走，导致「点同一个按钮暂停」失效。
...
断言行里 `[录音时间戳:: 27:04]` 的值即可点。`27:04`（27 分 04 秒）和 `00:29:00`（29 分）都认，分钟位最多三位（`135:00` 可以，`1350:00` 不识别）。
