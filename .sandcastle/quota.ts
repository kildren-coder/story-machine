...
// .exp/2026-07-21-afk-unattended-automation.md 第 4.1 节)。已查证堵死的路,别再
// 走一遍:`claude` CLI 没有 usage 子命令;transcript JSONL 不含 rate limit 字段;
...
// 落盘文件路径可覆盖:验收要求「用伪造的过期 JSON 验证起点按 0% 重算」,
// 有这个开关就不用去动真文件。
...
//
// **这个默认值是一阶估算,不是实测值**——与 agent-alert 共用同一把尺(同一订阅、
// 同一模型),那边 .env 里标定过的 AFK_UNITS_PER_PERCENT 直接拷过来用;要自己标定,
// 拿「本次报的单位数 ÷ statusline 上 5h% 的实际涨幅」即可。
// 故意取偏小的值:单位/% 越小,估算涨得越快,越早收工——宁可少跑一票,不要撞墙。
...
// 一轮 = 几十次 API 请求,不是一次。sandcastle 的 `iteration.usage` 按它自己的
// 文档只是"该轮**最后一条** assistant 消息"的快照——那是收尾时的 context 大小,
// 不是这一轮花掉的钱。而 agent 每调一次工具就是一次完整请求,整个 context 要
// 重发一遍。所以真实开销 ≈ Σ(每次请求的 context),差的正好是请求次数那个量级。
//
// 实测(2026-07-21,一个 37 请求的会话):累计 cache 读 212 万 token,而最后一条
// 只有 8.4 万——**低估 25 倍**;折成加权单位是 456k vs 13k,低估 35 倍。#30 那次
// 报"50k 单位"而实际烧掉 31% 额度,就是这么来的。
//
// 真账在会话 JSONL 里,每条 assistant 消息带自己那次请求的 usage。同一条消息会
// 因流式分片重复出现多行,按 message.id 去重。
...
// 别拿下一票去撞同一堵墙。
// 2026-07-21 #30 那次真撞墙,实际文案是
//   `You've hit your session limit · resets 11:40pm (UTC)`
// ——下面前四条一条都没匹配上,于是被归成"这个 issue 做失败了"。在 --loop 下
// 这是最坏的误判:循环会接着取下一票,再撞一次同一堵墙。真文案优先放前面。
const EXHAUSTED = [
  /session limit/i,
