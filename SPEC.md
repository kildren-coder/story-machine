...
- 一章一次调用。输入：头（集号、本章 `id` 与标题、本章的起止时刻与首末行号、说话人）+ 全集章节地图（每章的序号、起止时刻、`title`、`gist`，标出哪一章是本章）+ 本章切片（§5.2：本章自己的行，加前后各 2 分钟的行，分三段标明「上文」「本章」「下文」）。
...
- schema：`kind` 是三档之一；`line` 限在本章首末行号之间；`quotes` ≤ 6 条；各 `ts` 形如 `HH:MM:SS`；各键齐全、无多余键。
...
- 输出：每个话题一份 `frag-<topic>.json`（§5.4）。每整理完一章，代码先写这一章各话题的片段，再重写话题表 `topics.json`（§5.3：全部已完成章节的话题，按 `start` 排）。一章算完成 = 话题表里有它的话题，且这些话题的片段文件都在；重跑一章前先删掉 `chapter` 等于它的旧片段。
- 单章失败进 `_failed/`，不影响其他章。
- sonnet / medium，并发 ≤ 3。
...
话题表是 L2 每整理完一章由代码重写的索引：各片段（§5.4）去掉五类内容剩下的头，全集按 `start` 排。`id` = `<章节 id>-NN`。`kind`：`talk` / `aside` / `filler`。`gist` 由 L2 的模型写，一两句话说清这个话题讲了什么，用主播自己的说法。章内话题首尾相接铺满这一章；两个话题写了同一行时前一个长度为 0。provenance 的 `unit` 为 `all`，`derived_from` 为 `chapters.json` 与 `frag-*.json`。
...
**单集整理稿**渲染在 EP 笔记 `## 整理稿` 下：按话题顺序放 `paras`、原话锚点、可核查的说法、提到的信源、疑似 ASR 生音；时间戳用裸 `[HH:MM:SS]`。整理稿连同 `## 整理稿` 标题写在 `<!-- digest:auto -->` … `<!-- /digest -->` 标记块内：首次插在 `<!-- /speakers -->` 之后（没有则 `<!-- /ep -->` 之后，再没有则文末）；已存在则整块替换；块外字节不动。块内第一行写明「本块由 L3 渲染（整理版本、生成时间）；重跑会覆盖，批注请写在块外」。`filler` 话题不渲染，块内第一行补一句未渲染的段数与合计时长。frontmatter 只写 `整理:` 与 `整理版本:` 两个键，没有则新增在末尾，其他键与顺序不动；`整理版本:` 取渲染所依据那一层的 prompt 版本；块内第一行的生成时间取该层产物的 `provenance.generated_at`，产物未重生成时重渲染，块逐字节不变。

L2 上线前整理稿的形状：只有章节大纲——每章一行 `### [HH:MM:SS] 标题`（时间戳是该章的 `start`），下面一行 `gist`；`整理版本:` 取 L1 的 prompt 版本。
