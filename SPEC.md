...
- 代码检查：§5.3 的字段与类型；`id` 匹配 `^[a-z0-9-]+$` 且全集唯一；每个 `start` 读得出时刻、不超过整集时长、按话题顺序严格递增。不过按 §4.1 重试后进 `_failed/`。
- 代码归一，不打回：`start` 写成 `MM:SS` 按分秒读，统一补成 `HH:MM:SS`；第一个话题的 `start` 一律归零。起点落在两行之间不算错——行首每 30 秒才有一个，而话题边界本来只有这个精度。
- 覆盖整集由构造保证，不设闸门：`end` 由下一个话题的 `start` 推出，最后一个收在时长，所以空洞、重叠、零长度、倒置、越界都不可能出现。
- sonnet / low。
