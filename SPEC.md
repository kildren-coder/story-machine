...
- 代码检查只拦代码修不了的。能机械归一而不碰模型写的字的（时间戳格式、首尾边界、缺省值、代码本来就知道的键）由代码归一，不打回；模型照样违反的 prompt 禁令改成代码容忍；凡代码能推出来的（终点、集号）不让模型写。每层上线前用 3 集各跑 3 轮量首发通过率，目标 100%（L1：`scripts/bench_l1.py`，每轮只发一趟，产物落在仓库外；`--rejudge` 用当前代码重判已有响应，不调模型）。
...
- 输入：整集逐字稿，§5.2 格式，前面加三行头，一行一个键（`episode:` 只写集号、`时长:`、`说话人:`）。整集时长取逐字稿最后一段的 `end` 取整秒。
- prompt：`prompts/L1-skeleton.md`。
- 输出：`topics.json`（§5.3）。
- 规则：切在换论点的地方——判据是「这一段从头到尾在讲同一个论点」，同一个人、同一件事换个角度谈就该切开；不到 3 分钟又不是纯过场的并进相邻话题（子论点、例子、插补跟着所属论点走）；被打断后接回来的拆成两个话题（L4 会归回同一件事）。不按时长切。`title` 与 `gist` 内不出现引号与反斜杠。
- `kind` 三档，全部照列：正题标 `talk`；有内容但不是正题的（观众问答、念弹幕里的实质问题、离题闲聊见闻）标 `aside`；零信息的纯过场（答谢礼物与打赏名单、设备测试、问好寒暄、结尾预告、口播广告）标 `filler`。
- 代码检查：§5.3 的字段与类型；`id` 匹配 `^[a-z0-9-]+$` 且全集唯一；每个 `start` 读得出时刻、不超过整集时长。不过按 §4.1 重试后进 `_failed/`。
- 代码归一，不打回：`id` 转小写、下划线与空格换成短横线；`start` 写成 `MM:SS` 按分秒读，统一补成 `HH:MM:SS`；话题按 `start` 稳定排序（模型偶尔把回头再谈的话题按主题挪到前一次旁边），重排了几个记日志；第一个话题的 `start` 一律归零。起点落在两行之间不算错——行首每 30 秒才有一个，而话题边界本来只有这个精度。两个话题 `start` 相同算过：前一个话题长度为 0，L2 切片靠前后各 2 分钟的余量兜住。
- 覆盖整集由构造保证，不设闸门：`end` 由下一个话题的 `start` 推出，最后一个收在时长，所以空洞、重叠、倒置、越界都不可能出现。
- sonnet / low。
...
- 规则：只用切片内容；不引入外部知识纠正主播；只删口水不删内容。按 `kind` 分三档：`talk` 出全部字段；`aside` 只出 `paras`，把是什么事讲清楚，不展开衍生、不做分析，**不设字数上限**；`filler` **不调模型**，不出片段。
- sonnet / medium，并发 ≤ 3。
...
4. **schema** 合 §5.4。
5. **覆盖与 ASR 条目**：所有 `talk` 话题有片段；`asr[].heard` 逐字出现在切片里，否则该条删除并计数。

闸门只删、只警、只补 `quotes[].ctx`，不改任何模型写的字。结果写 `_digest/EP{n}/gates.json`：每话题的删除条数、越界条数、`hedge_missing` 列表、schema 结果，整集缺片段的 `talk` 话题列表。

闸门 1 / 2 / 5 有删除仍算通过（删后渲染）；schema 不过或 `talk` 话题缺片段算失败，EP 笔记 `整理:` 置 `failed`、不渲染。通过后渲染整理稿进 EP 笔记正文（§5.7），EP 笔记 `整理:` 置 `done`、`整理版本:` 置 L2 的 prompt 版本。
...
行的切法：每 30 秒一个窗（窗起点为整 30 秒），窗内同一说话人的段连成一行，段间用空格；窗内说话人变化时另起一行并写名字；每行以该行第一段的 `start` 开头。说话人名来自 EP 笔记 `[SPEAKER_XX:: 名字]`，没点名的保留 `SPEAKER_XX`。L2 切片 = 话题各时间范围前后各扩 2 分钟（越过整集首尾则截断），取起点落在范围内的段，段不切开；多段范围各切各的、按时间顺序拼接。
...
模型只出 `start`（`HH:MM:SS`，取自行首时间戳附近即可）。`ep` 与 `end` 由代码填：`end` 取下一个话题的 `start`，最后一个取时长。`kind`：`talk` / `aside` / `filler`。`gist` 是写给下一层的交接说明，把这一段在讲什么、重点在哪说清楚，用主播自己的说法。

### 5.4 话题片段 `frag-<topic>.json`

```json
{"id": "midterm", "title": "…", "start": "01:04:32", "end": "01:17:56", "who": [...],
...
8. **参数打磨**：话题粒度、锚点数、每层 model / effort。
