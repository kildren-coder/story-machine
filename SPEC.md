...
- 输出：`topics.json`（§5.3）。
- 规则：切在换论点的地方——判据是「这一段从头到尾在讲同一个论点」，同一个人、同一件事换个角度谈就该切开；不到 3 分钟又不是纯过场的并进相邻话题（子论点、例子、插补跟着所属论点走）；被打断后接回来的写成多段 `ranges`，不拆成两个话题。不按时长切。`title` 与 `gist` 内不出现引号与反斜杠。
- `kind` 三档，全部照列：正题标 `talk`；有内容但不是正题的（观众问答、念弹幕里的实质问题、离题闲聊见闻）标 `aside`；零信息的纯过场（答谢礼物与打赏名单、设备测试、问好寒暄、结尾预告、口播广告）标 `filler`。时间范围覆盖整集不留空洞（代码检查）。
...
`kind`：`talk` / `aside` / `filler`。`gist` 是写给下一层的交接说明，把这一段在讲什么、重点在哪说清楚，用主播自己的说法。
