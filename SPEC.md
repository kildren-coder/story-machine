...
- 代码检查：§5.3 的字段与类型；`id` 匹配 `^[a-z0-9-]+$` 且全集唯一；每个 `start` 是输入里出现过的行首时间戳；第一个话题的 `start` 等于第一行的时间戳；`start` 按话题顺序严格递增。不过按 §4.1 重试后进 `_failed/`。
...
模型只出 `start`，而且必须是输入里出现过的行首时间戳（照抄，不自己算）。`ep` 与 `end` 由代码填：`end` 取下一个话题的 `start`，最后一个取时长。`kind`：`talk` / `aside` / `filler`。`gist` 是写给下一层的交接说明，把这一段在讲什么、重点在哪说清楚，用主播自己的说法。
