...
- 代码检查：`title` 与 `gist` 是非空字符串、不含 HTML 注释；`line` 在本章首末行号之间；`talk` 与 `aside` 的 `paras` 非空；`paras` 每段以 `[HH:MM:SS]` 开头，除 `<who>` `<hedge>` 外无其他尖括号标记；各 `ts` 读得出时刻。逐字命中与时间范围归 L3。
...
- 输出：每个话题一份 `frag-<topic>.json`（§5.4）。每整理完一章，代码先写这一章各话题的片段，再重写话题表 `topics.json`（§5.3：全部已完成章节的话题，按 `start` 排）。一章算完成 = 话题表里有它的话题，且这些话题的片段文件都在；重跑一章前先删掉 `chapter` 等于它的旧片段（按片段里的 `chapter` 键认，不按文件名）。
