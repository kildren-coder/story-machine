...
- 代码归一，不打回：`start` 写成 `MM:SS` 按分秒读，统一补成 `HH:MM:SS`；话题按 `start` 稳定排序（模型偶尔把回头再谈的话题按主题挪到前一次旁边），重排了几个记日志；第一个话题的 `start` 一律归零。起点落在两行之间不算错——行首每 30 秒才有一个，而话题边界本来只有这个精度。两个话题 `start` 相同算过：前一个话题长度为 0，L2 切片靠前后各 2 分钟的余量兜住。
